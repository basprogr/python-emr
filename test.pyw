...
def main():   
    def scan(opt):      
        ...
        try: 
            ...
            if usia_match := re.search(r"\((\d+)\s+th", text_biasa):
                ...
                usia_STRING = int(usia_match.group(1)) 
            ...
            if opt == 'i':   
                ...
                if suhu_match:
                    raw_suhu = suhu_match.group(1).strip()  
                    angka_suhu = re.search(r"\d+[.,]?\d*", raw_suhu)
                    if angka_suhu:
                        comaToPeriod = angka_suhu.group(0).replace(",", ".")
                        suhu_INPUT.delete(0, tk.END)
                        suhu_INPUT.insert(0, comaToPeriod)
 
                rr_match = re.search(r"Pernafasan(.*?)Tensi", text_tabel, flags=re.IGNORECASE)
                if rr_match:
                    raw_rr = rr_match.group(1).strip()
                    angka_rr = re.search(r"\d+", raw_rr) 
                    if angka_rr:
                        rr_INPUT.delete(0, tk.END)
                        rr_INPUT.insert(0, angka_rr.group(0))
 
                spo2_match = re.search(r"SPO2(.*?)Suhu", text_tabel, flags=re.IGNORECASE)
                if spo2_match:
                    raw_spo2 = spo2_match.group(1).strip()
                    angka_spo2 = re.search(r"\d+", raw_spo2) 
                    if angka_spo2:
                        spo2_INPUT.delete(0, tk.END)
                        spo2_INPUT.insert(0, angka_spo2.group(0))  
             
                alergi_match = re.search(r'Alergi[ \t]*([^\r\n]+)?', text_tabel, flags=re.IGNORECASE) 
                alergi_INPUT.delete(0, tk.END)  
                alergi_val = alergi_match.group(1) if alergi_match and alergi_match.group(1) else ""
                alergi_val = alergi_val.strip() 
                if alergi_val and alergi_val.lower() != "tidak":
                    alergi_removeYa = alergi_val.replace('Ya, ', '').replace('Bahan Alergen :', '')
                    alergi_INPUT.insert(0, alergi_removeYa.strip()) 
                else:
                    alergi_INPUT.insert(0, '-')
        ...
        try: 
            ...
            for line in input_text.splitlines():
                if not line.strip(): 
                    continue 
                parts = line.split('-')
                complete_parts = [parts[i] if i < len(parts) else defaults[i] for i in range(9)]
            ...
            for line in lines:
                if line.strip():
                    ...
                    b = tk.Button(routineFieldset, text=f'{room}', font=(ff, fs), command=lambda l=line: routine('all', l))
                    b.pack(fill='x', pady=2, padx=2)  
            generateButton.config(state='normal')
  
    def vitalSign(opt, row=None): 
        if opt == 'new' :
            pyautogui.write(rr_INPUT.get())
            pyautogui.press('tab')  
            pyautogui.write(spo2_INPUT.get())
        ...
        if opt == 'routine' :
            ...
            # mekanisme jika pasien tidak penurunan kesadaran
            if respon == 0 :
                pyautogui.press('tab')
                pyautogui.press('space')
                pyautogui.press('down')
    ...
    def routine(opt, line=None) :
        if opt == 'all' :
            shift = 'morning' if 6 < currentHour < 14 else 'notMorning'  
            time.sleep(1)
            openLink('ttv') 
            notify('Isi TTV?')
            while True: 
                time.sleep(1)
                vitalSign('routine', line)
    ...
    def run_all_routine(): 
        runAllButton.config(state="disabled") 
        try: 
            input_text = txa_vitalSign.get("1.0", tk.END).strip()
            if not input_text:
                return
            ...
            while True:
                routine('handover') 
                confirm = messagebox.askokcancel('Notifikasi', 'tekan cancel untuk mengulang handover, OK untuk selesai') 
                if confirm:
                    break  
            ...
            runAllButton.config(state="normal")
    ...
    handoverButton = tk.Button(fset_handover, font=(ff, fs), text="fill out", command=lambda: routine('handover'))
